...
ENT.Information = [[Drops FlashBang.]]
...
if (SERVER) then
	...
	function ENT:StartTouch( ent )
		...
		if ent:HasWeapon( "weapon_hns_flashbang" ) then return end		--We dont want them stocking up on grenades.
		if table.HasValue(ent.powerUps, "FlashBang") then return end	--Gets cleared on player spawn.
		
		if ( IsValid(ent) and ent:IsPlayer() and ent:Team() != TEAM_SEEKER ) then
			table.insert(ent.powerUps, "FlashBang")
			ent:Give( "weapon_hns_flashbang" )
		end
