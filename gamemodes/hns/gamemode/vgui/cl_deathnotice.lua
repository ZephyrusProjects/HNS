...
killicon.Add("default", "hns/cstrikeHUD/death_icon", Color_Icon)
killicon.Add("suicide", "hns/cstrikeHUD/death_icon", Color_Icon)
killicon.Add("worldspawn", "hns/cstrikeHUD/death_icon", Color_Icon)
--killicon.Add("env_physexplosion", "hns/cstrikeHUD/he_icon", Color_Icon)
killicon.Add("weapon_hns_knife", "hns/cstrikeHUD/knife_icon", Color_Icon)
killicon.Add("weapon_hns_awp", "hns/cstrikeHUD/awp_icon", Color_Icon)
killicon.Add("weapon_hns_deagle", "hns/cstrikeHUD/deagle_icon", Color_Icon)
killicon.Add("hns_grenade_proj", "hns/cstrikeHUD/he_icon", Color_Icon)
killicon.Add("headshot", "hns/cstrikeHUD/head-shot_icon", Color_Icon)
...
function GM:AddDeathNotice( Victim, team1, Inflictor, Attacker, team2, headshot )
	...
	else Death.color1 = table.Copy( team.GetColor( team1 )) 
	Death.color11 =	Color(0,0,0,225) end
		
	if ( team2 == -1 ) then Death.color2 = table.Copy( NPC_Color ) 
	else Death.color2 = table.Copy( team.GetColor( team2 ) )
	Death.color11 =	Color(0,0,0,225) end
	...
	--Rename default world damage
	if Inflictor == "worldspawn" then
		Death.left = "Hard Landing"
	end
	
	--Rename the env_explosion
	if Inflictor == "env_physexplosion" then
		Death.left = Attacker
	end
	
	--Removes players name if the grenaded themselved
	if (Death.left == Death.right) then
		Death.left = nil
	end
...
local function DrawDeath( x, y, death, hud_deathnotice_time )
	...
	-- Draw KILLER
	if (death.left) then
		draw.SimpleText( death.left, 	"MenuLarge1", x - (w/2) +  1, y + 1, 		death.color11, 	TEXT_ALIGN_RIGHT )
		draw.SimpleText( death.left, 	"MenuLarge1", x - (w/2), y, 		   		death.color1, 	TEXT_ALIGN_RIGHT )
	end
	
	-- Draw VICTIM
	draw.SimpleText( death.right, 		"MenuLarge1", x + (w/2) + 1, y + 1, 		death.color11, 	TEXT_ALIGN_LEFT )
	draw.SimpleText( death.right, 		"MenuLarge1", x + (w/2) , y, 		death.color2, 	TEXT_ALIGN_LEFT )
